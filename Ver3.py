...
class Agent:
    def __init__(self, x: float, y: float, team: str):
        ...
        self.velocity_y = 0
        self.team = team
        self.speed = 1.0  # Reduced from 2.0
        self.size = 8
        self.carrying_resource = None
        self.perception_radius = 150
        self.communication_radius = 200
        self.target = None
        self.role = 'searching'  # 'searching', 'gathering', 'returning', 'resting'
        self.energy = 100
        self.waiting_time = 0
        self.collaborators = []
        self.last_position = (x, y)
        self.stuck_time = 0
        self.stuck_escape_direction = None
    ...
    def update(self, resources: List[Resource], agents: List['Agent'], base_x: float, base_y: float):
        # Energy management
        if self.energy <= 20:  # Start resting when energy is low
            self.role = 'resting'
            if self.carrying_resource:
                # Safely drop resource if carrying one while tired
                resource = self.carrying_resource
                if resource:
                    # Safely remove self from carriers if present
                    if self in resource.carriers:
                        resource.carriers.remove(self)

                    # If no carriers left, reset resource state
                    if len(resource.carriers) == 0:
                        resource.collected = False

                    # Reset agent's resource reference
                    self.carrying_resource = None

        # Resting behavior
        if self.role == 'resting':
            self.velocity_x = 0
            self.velocity_y = 0
            self.energy = min(100, self.energy + 0.1)  # Slower energy recovery
            if self.energy >= 90:  # Only return to work when mostly recovered
                self.role = 'searching'
            return  # Skip other updates while resting

        # Energy consumption based on activity
        energy_cost = 0.02  # Base energy cost
        if self.carrying_resource:
            energy_cost *= 1.5  # Higher energy cost when carrying
        if abs(self.velocity_x) > 0 or abs(self.velocity_y) > 0:
            energy_cost *= 1.2  # Additional cost when moving
        self.energy = max(0, self.energy - energy_cost)

        # Rest of the update logic remains the same...
        self.nearby_agents = agents
        ...
        avg_movement = total_movement / (len(self.last_positions) - 1)

        if avg_movement < 0.5:  # Reduced threshold
            self.stuck_time += 1
            if self.stuck_time > 30:
                if not self.stuck_escape_direction:
                    angle = random.uniform(0, 2 * math.pi)
                    self.stuck_escape_direction = (
                        math.cos(angle) * self.speed * 1.5,  # Reduced escape speed
                        math.sin(angle) * self.speed * 1.5
                    )

                self.velocity_x = self.stuck_escape_direction[0]
                self.velocity_y = self.stuck_escape_direction[1]

                if self.stuck_time > 90:
                    self.role = 'searching'
                    self.target = None
                    self.stuck_time = 0
                    self.stuck_escape_direction = None
                    self.x += random.uniform(-25, 25)  # Reduced random movement
        ...
        self.x += self.velocity_x * 0.7  # Speed multiplier
        self.y += self.velocity_y * 0.7

        # Keep within bounds with bounce behavior
        if self.x <= 0 or self.x >= WINDOW_WIDTH:
            self.velocity_x *= -0.8  # Reduced bounce speed
            self.x = max(0, min(self.x, WINDOW_WIDTH))
        if self.y <= 0 or self.y >= WINDOW_HEIGHT:
            self.velocity_y *= -0.8
            self.y = max(0, min(self.y, WINDOW_HEIGHT))

        # Team-specific behavior with adjusted speeds
        if self.team == 'collector':
            if self.carrying_resource:
                self.role = 'returning'
                self.move_towards(base_x, base_y)
                self.speed = 0.7  # Slower when carrying
                ...
                self.speed = 1.0  # Normal speed when not carrying
                if self.role == 'searching':
            ...
            if target:
                self.speed = 1.2  # Slightly faster than collectors
                self.update_opposer_movement(target)
            else:
                self.speed = 0.8  # Slower patrol speed
        ...
        # Apply swarm rules with reduced effect
        ...
        def move_towards(self, target_x: float, target_y: float, slow_down_radius=50):
            ...
            if distance > 0:
                speed_factor = min(0.8,
                                   distance / slow_down_radius) if distance < slow_down_radius else 0.8  # Reduced speed factor
                self.velocity_x = (dx / distance) * self.speed * speed_factor
                self.velocity_y = (dy / distance) * self.speed * speed_factor
...
class Simulation:
    def __init__(self):
        ...
        self.opposers = [Agent(random.randint(0, WINDOW_WIDTH),
                               ...
                               'opposer') for _ in range(5)]
        self.agents = self.collectors + self.opposers
        self.spawn_cooldown = 300  # Increased spawn cooldown (5 seconds at 60 FPS)
...
#resource spawn
            spawn_timer += 1
            if spawn_timer >= self.spawn_cooldown:  # Increased from 180 to 300
                resource_type = random.choice(list(ResourceType))
                while True:
...
#slow and tend to cluster and stuck. but refilling energy